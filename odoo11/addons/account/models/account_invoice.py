...
class AccountInvoice(models.Model):
    ...
    delivery_check = fields.Boolean(string='Include Vat(for printing)', default=False, readonly=False, copy=False)
    exvat = fields.Boolean(string='Show exvat only', default=False, readonly=False, copy=False)
    ...
    f_2306 = fields.Boolean(string="2306", default=False)
    f_2307 = fields.Boolean(string="2307", default=False)
    ...
    @api.multi
    @api.onchange('f_2306', 'f_2307')
    def _field_restrict(self):
        managers = self.env['res.groups'].search([('name', '=', 'Billing Manager')])
        if self.env.user.id not in managers.users.ids:
            raise UserError(_("You should be a billing manager to confirm payment."))
    ...
    @api.multi
    def formatted_invoice_print(self):
        """Print formatted version of delivery report
        """
        return self.env.ref('account.delivery_report_formatted').report_action(self)
...
class AccountInvoiceLine(models.Model):
    ...
    uom_id_2 = fields.Char(string='Unit of Measure(Printed)', help="Customer facing unit of measure", required=False)
    ...
    def _onchange_uom_id(self):
        warning = {}
        result = {}
        self.uom_id_2 = self.uom_id.name
        if not self.uom_id:
            self.price_unit = 0.0
